...
所有联邦学习API接口都需要用户身份验证和权限控制：

#### 访问权限
- **ADMIN（管理员）**: 拥有所有联邦学习接口的完整访问权限
- **RESEARCHER（研究员）**: 拥有所有联邦学习接口的完整访问权限
- **OPERATOR（操作员）**: 拥有所有联邦学习接口的完整访问权限
- **VIEWER（观察者）**: ❌ **禁止访问** 所有联邦学习接口

#### 认证要求
- 所有请求必须在Header中携带有效的JWT Token
- Token格式：`Authorization: Bearer <token>`
- Token必须为`access`类型（非refresh类型）
- Token中必须包含有效的用户角色信息
...
> ℹ️ **提示**: 若需要切换为自定义初始模型，只需将 `mode` 设置为 `CUSTOM` 并提供 `initialModelId`，此时可忽略 `autoGenerateConfig`。
...
**v1.5 版本重要更新**:
- **🎯 任务创建流程调整**: 任务必须引用已上传的数据集(`datasetId`)，初始模型通过 `initialModelConfig` 绑定
- **🤖 初始模型模式**: 新增 `AUTO`/`CUSTOM` 两种初始模型模式，未提供 `initialModelId` 时后端自动生成默认模型
- **🗂️ 模型配置拆分**: 弃用旧的 `modelConfig`，由 `initialModelConfig.autoGenerateConfig` 承担自动生成时的配置职责
- **🔁 绑定查询增强**: 支持通过 `GET /api/model/initial/task/{taskId}` 查询任务所绑定的初始模型
...
- 🔄 **字段调整**: `modelConfig` 被 `initialModelConfig` 取代，必须按照新结构提交初始模型配置

**迁移要求**:
- 所有客户端必须在任务创建时提供有效的 `datasetId`
- 任务创建请求需改用 `initialModelConfig`，根据模式补充 `initialModelId` 或 `autoGenerateConfig`
- 前端代码需要适配新的分页字段名
- 详细修复内容请参考：[v1.5修改文档](../modified/modified-interfaces-v1.5.md) 与 [v1.4修改文档](../modified/modified-interfaces-v1.4.md)
...
### 3.1 任务创建接口 (v1.5 流程调整)

> 🚨 **重要变更**: v1.5 要求任务创建时引用已有数据集并通过 `initialModelConfig` 指定初始模型；同时继承 v1.4 的严格参数验证要求
...
```json
{
  ...
  // 🆕 v1.5: 初始模型配置（CUSTOM 引用已有模型，AUTO 由后端生成）
  "initialModelConfig": {
    "mode": "CUSTOM",                      // CUSTOM | AUTO
    "initialModelId": "initial_model_001", // CUSTOM 模式必填
    "autoGenerateConfig": {
      ...
      "description": "默认联邦随机森林初始模型"
    } // 当 mode = AUTO 时启用该配置；CUSTOM 模式会忽略此对象
  },
