...
v1.5及v1.5.1正式要求**虚拟机与后端均需维护完整的协议枚举**，并对来源于对端的所有消息类型实现显式处理逻辑。

- **后端要求**: `ProtocolType`枚举必须覆盖全部消息类型（含ACK、错误与数据集管理协议），`WebSocketProtocolService`需对来自虚拟机的每一种类型给出处理分支或明确的错误反馈。  
- **虚拟机要求**: 本地枚举同样需包含全量类型，能够正确处理后端推送的所有命令、ACK与错误消息，避免因遗漏而出现未识别消息。  
- **错误容错**: 对于`MESSAGE_ERROR`、`CONNECTION_ERROR`等双向协议，双方都要在接收后记录日志、返回ACK并执行恢复策略。  
- **实现提示**: 建议在代码中通过`switch/when`覆盖所有枚举值，同时添加默认分支记录“未知协议”告警，以便持续对齐后端更新。
