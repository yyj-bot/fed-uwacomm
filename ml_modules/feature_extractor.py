...
class BellhopFeatureExtractor:
    ...
    def extract_prt_features(self, prt_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            # Extract source depths
            src_match = re.search(r'Source depths \(m\):\s*([\d.\s]+)', content)
            if src_match:
                features['source_depths'] = [float(x) for x in src_match.group(1).split()]
            
            # Extract receiver depths
            rcv_match = re.search(r'Receiver depths \(m\):\s*([\d.\s-]+)', content)
            if rcv_match:
                features['receiver_depths'] = [float(x) for x in rcv_match.group(1).split()]
            
            # Extract ranges
            range_match = re.search(r'Receiver ranges \(km\):\s*([\d.\s-]+)', content)
            if range_match:
                features['ranges'] = [float(x) for x in range_match.group(1).split()]
            
            # Extract beam angles (for ray mode)
            beam_match = re.search(r'Beam angles \(degrees\):\s*([\d.\s-]+)', content)
            if beam_match:
                features['beam_angles'] = [float(x) for x in beam_match.group(1).split()]
            
            # Extract run time
            time_match = re.search(r'CPU time:\s*([\d.]+)', content)
            if time_match:
    ...
    def extract_shd_features(self, shd_file: Path) -> Dict[str, Any]:
        ...
        try:
            with open(shd_file, 'rb') as fid:
                ...
                # Read transmission loss data (simplified sampling)
                if Nsz > 0 and Nrz > 0 and Nrr > 0:
                    tl_values = []
                    
                    # Sample data to avoid memory issues
                    sample_size = min(1000, Nrz * Nrr)
                    
                    for i in range(0, min(Nsz, 1)):  # Only first source
                        for j in range(0, min(Nrz, 50), max(1, Nrz//50)):  # Sample depths
                            recnum = 10 + i * Nrz + j
                            fid.seek(recnum * 4 * recl, 0)
                            temp = struct.unpack(f'<{2*Nrr}f', fid.read(8 * Nrr))
                            
                            for k in range(0, len(temp), max(2, len(temp)//50)):  # Sample ranges
                                if k + 1 < len(temp):
                                    real_part = temp[k]
                                    imag_part = temp[k + 1]
