...
const UserVmList: React.FC<UserVmListProps> = ({ userId, username }) => {
  ...
  const handleBatchRemove = async () => {
    if (selectedRowKeys.length === 0) {
      message.warning('请选择要移除的水下节点')
      return
    }

    Modal.confirm({
      title: '确认批量移除',
      content: `确定要移除 ${selectedRowKeys.length} 台水下节点的分配吗？`,
      onOk: async () => {
  ...
  return (
    ...
    >
      <Table
        ...
        locale={{ emptyText: '该用户暂未分配任何水下节点' }}
        loading={loading}
      />

      {/* 批量分配模态框 */}
      <Modal
        title={`批量分配水下节点给 ${username}`}
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item
            label="选择水下节点"
            name="vmIds"
            rules={[{ required: true, message: '请选择至少一台水下节点' }]}
          >
            <Select
              mode="multiple"
              placeholder="请选择水下节点"
